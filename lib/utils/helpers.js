...
async function upsertObject(msg, token, objectExists, type, meta, cfg) {
  ...
  if (objectExists) {
    ...
    method = "PUT";
    uri = `https://app.staffomatic.app/v3/${cfg.company_domain}/${type}/${meta.recordUid}`;
    newObject = prepareObject(msg, type);
    delete newObject.uid;
  } else {
    // Create the object if it does not exist
    method = "POST";
    uri = `https://app.staffomatic.app/v3/${cfg.company_domain}/${type}`;
    newObject = msg;
    delete newObject.uid;
...
async function createObject(msg, token, type, company_domain) {
  if (!type) {
    return false;
  }
  ...
  // Create the object if it does not exist
  method = "POST";
  uri = `https://app.staffomatic.app/v3/${company_domain}/${type}`;
  newObject = msg;
  console.log(msg)
  // delete newObject.uid;
  // delete newObject.categories;
  ...
  try {
    ...
    const person = await request(options);
    console.log("this is the response",person.body)
    return person.body;
  } catch (e) {
    return e;
  }
...
async function getEntries(token, snapshot, type, company_domain) {
  ...
  //   uri = `${BASE_URI}/${type}`;
  // }
  uri = `https://app.staffomaticapp.com/v3/${company_domain}/${type}`;
  try {
