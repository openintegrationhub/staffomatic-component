...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    ...
    const absences = await getEntries(token, snapshot, "absences", cfg.company_domain);

    console.log(`Found ${absences.length} new records.`);

    const newElement = {};
    newElement.metadata = oihMeta;
    newElement.data = absences;
    for(let i = 0; i < newElement.data.length; i++ ){
           const newObject = newElement;
           newObject.data = newElement.data[i];
           self.emit("data",newObject)
       } 
